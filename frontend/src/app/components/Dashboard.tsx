...
export function Dashboard() {
  ...
  const clearAllAlerts = async () => {
    ...
    if (acknowledgedAlerts.length === 0) {
      addNotification('error', 'No acknowledged alerts to clear. Acknowledge alerts before clearing.');
      return;
    }
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/clear_all_alerts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ acknowledged_ids: acknowledgedAlerts.map(a => a.id) })
      });

      const payload = await response.json().catch(() => ({}));
      if (response.ok && payload.status === 'success') {
        // Keep only unacknowledged alerts
        const remaining = alerts.filter(a => !a.acknowledged);
        setAlerts(remaining);
        alertsRef.current = remaining;
        addNotification('success', `Cleared ${acknowledgedAlerts.length} acknowledged alert(s). ${remaining.length} unacknowledged alert(s) remain.`);
      } else {
            ...
            <div className="relative h-48 flex items-center justify-center">
              ...
              {[0, 60, 120, 180, 240, 300].map((angle, index) => {
                ...
                return (
                  <motion.div
                    key={angle}
                    className={`absolute w-10 h-10 rounded-full border-2 flex items-center justify-center cursor-pointer ${status === "critical" ? "bg-red-500/20 border-red-500" :
            ...
            <div className="relative py-4">
              {timelineStages.map((stage, index) => (
                <div key={index} className="relative flex items-center gap-4 mb-6 last:mb-0">
                  <motion.div
                    className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${stage.active ? `bg-${stage.color}-500/20 border-${stage.color}-500` : "bg-gray-500/10 border-gray-500"
                      }`}
          ...
          <div className="grid grid-cols-4 gap-4">
            {displayedFleet.map((engine) => (
              <motion.div
                ...
                className={`p-4 rounded-xl border-2 cursor-pointer transition-all relative overflow-hidden ${engine.maintenance_status === "in-progress" ? "bg-cyan-500/10 border-cyan-400 border-dashed animate-pulse" :
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  {engine.maintenance_status ? (
                    <span className={`text-[10px] uppercase font-bold tracking-wide px-2 py-0.5 rounded-full ${engine.maintenance_status === "in-progress" ? "bg-cyan-500/20 text-cyan-300 border border-cyan-500/40" :
                        "bg-orange-500/20 text-orange-300 border border-orange-500/40"
                      }`}>
                      {engine.maintenance_status === "in-progress" ? "Servicing" : "Queued"}
                    </span>
                  ) : (
                    <Radio className={`w-5 h-5 ${engine.status === "critical" ? "text-red-400" :
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className={`font-bold ${engine.status === "critical" ? "text-red-400" :
                        engine.status === "warning" ? "text-orange-400" : "text-green-400"
                      }`}>{engine.health}%</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
      ...
      <div className="space-y-3">
        {alerts.filter(alert => {
          ...
          <motion.div
            ...
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className={`p-4 rounded-xl border-2 ${alert.severity === "critical" ? "bg-red-500/10 border-red-500/50" :
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${alert.severity === "critical" ? "bg-red-500/20 text-red-400" :
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span>{task.date}</span>
                        <span className={`px-2 py-0.5 rounded-full ${task.priority === "high" ? "bg-red-500/20 text-red-400" :
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg border text-xs font-medium transition-all ${selectedMaintenanceTask?.id === task.id
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${task.status === 'completed' ? 'bg-green-500/20 text-green-400' :
            ...
            return (
              <div key={item.id} className="relative group">
                <motion.button
                  ...
                  className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${activeNav === item.id ? "bg-cyan-500/20 border border-cyan-500/50" : "bg-white/5 border border-white/10 hover:bg-white/10"
                    }`}
      ...
      <div className="ml-16 h-full flex flex-col">
        ...
        <div className="h-16 bg-[#0d1117] border-b border-cyan-500/20 px-6 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <h1 className="text-xl font-bold text-white tracking-wider">AI-POWERED PREDICTIVE MAINTENANCE SYSTEM</h1>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-500/10 border border-green-500/30">
              ...
              <AnimatePresence>
                {showUserMenu && (
                  ...
                  >
                    ...
                    {/* Menu Items */}
                    <div className="py-2 px-4 border-b border-white/10 text-xs text-gray-400 space-y-1">
                      <div>Login Timing:</div>
                      <div className="font-mono text-cyan-400">
                        {currentUser?.loginTime ? new Date(currentUser.loginTime).toLocaleString() : "N/A"}
                      </div>
                    </div>
                    <div className="py-2">
            ...
            >
              ...
              <div className="p-6 border-b border-cyan-500/20">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {loadingStates['clear-all'] ? 'Clearing...' : 'Clear Acknowledged'}
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-auto p-4 space-y-3">
                {alerts.map((alert, index) => (
                  <motion.div
                    ...
                    className={`p-4 rounded-xl border-2 ${alert.severity === "critical"
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <button
                          ...
                          className={`px-4 py-3 rounded-lg font-medium transition-all ${selectedReportFormat === 'pdf'
                        ...
                        <button
                          ...
                          className={`px-4 py-3 rounded-lg font-medium transition-all ${selectedReportFormat === 'excel'
                              ? 'bg-cyan-500/20 border border-cyan-500/50 text-cyan-400'
                              : 'bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10'
                            }`}
                        >
                        ...
                        <button
                          ...
                          className={`px-4 py-3 rounded-lg font-medium transition-all ${selectedReportFormat === 'csv'
                              ? 'bg-cyan-500/20 border border-cyan-500/50 text-cyan-400'
                              : 'bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10'
                            }`}
                        >
        ...
        <AnimatePresence>
          {notifications.map((notification) => (
            <motion.div
              ...
              className={`p-4 rounded-lg border-2 shadow-lg ${notification.type === 'success'
                  ? 'bg-green-500/10 border-green-500/50'
                  : 'bg-red-500/10 border-red-500/50'
                }`}
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm font-medium ${notification.type === 'success' ? 'text-green-400' : 'text-red-400'
                  }`}>
